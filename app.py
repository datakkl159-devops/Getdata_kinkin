...
COL_BLOCK_NAME = "Block_Name" # Cột mới để phân luồng

SCOPES = ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']
DEFAULT_BLOCK_NAME = "Block_Mac_Dinh" # Tên khối cho dữ liệu cũ
            ...
            for row in group_rows:
                ...
                if any(t[1] == s_link for t in tasks_list) or (s_link in global_results_map and "Lỗi" in global_results_map[s_link][0]):
                    ...
                    log_entries.append([
                        ...
                        row.get('Tên sheet nguồn dữ liệu gốc', ''), status_str, height, final_range 
                    ])
...
def main_ui():
    ...
    def load_data(creds):
        ...
        # Gán Block Mặc Định cho dữ liệu cũ (chưa có cột Block_Name)
        ...
        # 2. Load Sys Config (Blocks & Schedule)
        ...
        # Nếu là file cấu hình kiểu cũ (key-value), reset về format bảng mới
        ...
        # Đảm bảo có cấu hình cho khối mặc định
        if DEFAULT_BLOCK_NAME not in df_sys['Block_Name'].values:
             ...
             st.toast(f"Đã tự động tạo cấu hình cho {DEFAULT_BLOCK_NAME}!")
    ...
    for block_name in all_blocks:
        ...
        with st.container(border=True):
            ...
            with c_head: 
                display_name = f"📦 Khối: {block_name}"
                if block_name == DEFAULT_BLOCK_NAME: display_name += " (Mặc định)"
                st.subheader(display_name)
                
            with c_btn:
                if block_name != DEFAULT_BLOCK_NAME: # Không cho xóa khối Default
                    if st.button("🗑️ Xóa Khối", key=f"del_{block_name}"):
                        ...
                        # (Tuỳ chọn: Xóa luôn dữ liệu trong config thuộc khối này nếu muốn)
                        save_sys_config(st.session_state['df_sys'], creds)
                        st.rerun()
            ...
            # Bảng dữ liệu (Data Editor) lọc theo Khối
            ...
            # Logic cập nhật ngược lại Main DataFrame
            if not edited_block_df.equals(df_block_view):
                edited_block_df['Block_Name'] = block_name # Gán khối cho dòng mới thêm
            ...
            with c_run_b:
                if st.button(f"▶️ Chạy Khối '{block_name}'", key=f"run_{block_name}"):
                    ...
                    if not rows_run: st.warning("Không có dòng nào 'Chưa chốt'.")
                        ...
                        with st.spinner(f"Đang xử lý khối {block_name}..."): # Thông báo đang xử lý
