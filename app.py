...
# 1. CẤU HÌNH HỆ THỐNG
# ==========================================
st.set_page_config(page_title="Kinkin Tool 2.0 (V100 - MultiCore)", layout="wide", page_icon="🚀")
...
BOT_EMAIL_DISPLAY = "System: Multi-Bot Engine Ready"
...
# --- ĐỊNH NGHĨA CỘT (GIỮ NGUYÊN TIẾNG VIỆT) ---
...
# [V100] Cột duy nhất thêm vào để phân tải
COL_ASSIGNED_BOT = "Bot_Phu_Trach"
...
# ==========================================
# 2. AUTHENTICATION (MULTI-BOT ENGINE)
# ==========================================
...
def get_available_bots():
    """Tự động quét Secrets để tìm danh sách Bot"""
    bots = {}
    # Bot 1
    if "gcp_service_account" in st.secrets:
        bots["Bot 1"] = get_master_creds()
    
    # Bot phụ (gcp_service_account_1, _2...)
    all_secs = st.secrets.to_dict() if hasattr(st.secrets, "to_dict") else dict(st.secrets)
    for key in all_secs:
        if key.startswith("gcp_service_account_"):
            try:
                idx = key.split("_")[-1] # Lấy số 1, 2
                bot_name = f"Bot {int(idx) + 1}" # Bot 2, Bot 3
                raw = all_secs[key]
                info = json.loads(raw) if isinstance(raw, str) else dict(raw)
                if "private_key" in info: info["private_key"] = info["private_key"].replace("\\n", "\n")
                bots[bot_name] = service_account.Credentials.from_service_account_info(info, scopes=SCOPES)
            except: pass
    
    # Sắp xếp để Bot 1 luôn đầu tiên
...
def safe_api_call(func, *args, **kwargs):
    ...
    for i in range(max_retries):
        try: return func(*args, **kwargs)
            ...
            if "429" in error_str or "quota" in error_str or "api" in error_str or "500" in error_str:
                wait_time = (2 ** i) + 3 
                print(f"⚠️ API Busy. Waiting {wait_time}s...")
...
def col_name_to_index(col_name):
    col_name = col_name.upper(); index = 0
...
def flush_logs(creds, force=False):
    ...
    if (force or len(buffer) >= LOG_BUFFER_SIZE or (time.time() - last_flush > LOG_FLUSH_INTERVAL)) and buffer:
        try:
            ...
            st.session_state['log_buffer'] = []; st.session_state['last_log_flush'] = time.time()
        except: pass
    ...
    for i in range(len(dfo)):
        for col in compare_cols:
            ...
            if val_old != val_new:
                ...
                if len(changes) >= 3: changes.append("..."); return " | ".join(changes)
    ...
    with c2:
        ...
        if st.button("Đăng Nhập", use_container_width=True):
            if pwd in AUTHORIZED_USERS:
                ...
                log_user_action_buffered(get_master_creds(), AUTHORIZED_USERS[pwd], "Đăng nhập", "Thành công", force_flush=True)
                st.rerun()
...
def acquire_lock(creds, user_id):
    try:
        ...
        except: wks = sh.add_worksheet(SHEET_LOCK_NAME, 10, 5); wks.update([["is_locked", "user", "time_start"], ["FALSE", "", ""]])
...
def fetch_data_v4(row_config, bot_creds, target_headers=None, status_container=None):
    # [V100] Dùng bot_creds được truyền vào
...
def check_permissions_ui(rows, creds, container, user_id):
    ...
    for r in rows:
        s_link = str(r.get(COL_SRC_LINK, '')).strip(); t_link = str(r.get(COL_TGT_LINK, '')).strip()
        if "docs.google.com" in s_link: src_links.add(s_link)
        if "docs.google.com" in t_link: tgt_links.add(t_link)
    all_unique_links = list(src_links.union(tgt_links)); total = len(all_unique_links)
    if total == 0: container.info("Không tìm thấy link Google Sheet nào."); return
    prog = container.progress(0); err_count = 0
    for i, link in enumerate(all_unique_links):
        prog.progress((i + 1) / total); time.sleep(0.2)
        ok, msg = verify_access_fast(link, creds)
        if not ok:
            err_count += 1; error_msgs = []
            if link in src_links: error_msgs.append("Link Nguồn: cần quyền XEM")
            if link in tgt_links: error_msgs.append("Link Đích: cần quyền SỬA")
            container.error(f"❌ {link}\n👉 {' & '.join(error_msgs)}")
    if err_count == 0: container.success("✅ OK: Bot có đủ quyền.")
...
def process_pipeline_mixed(rows_to_run, user_id, block_name_run, status_container):
    ...
    if not acquire_lock(master_creds, user_id): 
        st.error("⚠️ Hệ thống đang bận. Vui lòng thử lại sau."); return False, {}, 0
    
    # [V100] Lấy danh sách Bot
    available_bots = get_available_bots() # {'Bot 1': creds1, 'Bot 2': creds2...}
    if not available_bots: st.error("❌ Không tìm thấy Bot!"); return False, {}, 0
    
    log_user_action_buffered(master_creds, user_id, f"Chạy: {block_name_run}", "Đang xử lý...", force_flush=True)
    try:
        # Nhóm theo Bot -> File Đích
        grouped = defaultdict(list)
        for r in rows_to_run:
            if str(r.get(COL_STATUS, '')).strip() == "Chưa chốt & đang cập nhật":
                # Lấy Bot phụ trách dòng này
                bot_name = str(r.get(COL_ASSIGNED_BOT, "Bot 1")).strip()
                if bot_name not in available_bots: bot_name = "Bot 1" # Fallback
                
                # Group key: (Bot, Link Đích, Sheet Đích)
                key = (bot_name, str(r.get(COL_TGT_LINK, '')).strip(), str(r.get(COL_TGT_SHEET, '')).strip())
        ...
        for idx, ((bot_name, t_link, t_sheet), group_rows) in enumerate(grouped.items()):
            
            # Lấy Credentials của Bot này
            current_bot_email = available_bots[bot_name]
            # Hàm get_bot_creds_by_name cần được định nghĩa hoặc dùng logic này:
            if bot_name == "Bot 1": bot_creds = master_creds
            else:
                idx_b = int(bot_name.replace("Bot ", "")) - 1
                raw = st.secrets[f"gcp_service_account_{idx_b}"]
                info = json.loads(raw) if isinstance(raw, str) else dict(raw)
                if "private_key" in info: info["private_key"] = info["private_key"].replace("\\n", "\n")
                bot_creds = service_account.Credentials.from_service_account_info(info, scopes=SCOPES)

            with status_container.expander(f"[{bot_name}] Processing: ...{t_link[-10:]}", expanded=True):
                target_headers = []
                try:
                ...
                for i, r in enumerate(group_rows):
                    ...
                    df, sid, msg = fetch_data_v4(r, bot_creds, target_headers, status_container=msg_placeholder)
                    time.sleep(1.0)
                    
                    if df is not None: 
                        count = len(df)
                        msg_placeholder.success(f"✅ Tải xong: {lnk[-10:]} ({lbl}) | SL: {count} dòng")
                        tasks.append((df, lnk, row_idx, w_mode))
                        total_rows += len(df)
                    else: 
                        msg_placeholder.error(f"❌ Lỗi tải: {lnk[-10:]} | {msg}")
                        final_res_map[row_idx] = ("Lỗi tải", "", 0)
                    
                    del df; gc.collect()

                if tasks:
                    ok, msg, batch_res_map, batch_debug = write_strict_sync_v2(tasks, t_link, t_sheet, bot_creds, st)
        ...
        log_user_action_buffered(master_creds, user_id, f"Kết quả chạy {block_name_run}", status_msg, force_flush=True)
...
def load_full_config(_creds):
    ...
    # [V100] Tự động điền Bot nếu trống
    if COL_ASSIGNED_BOT not in df.columns: df[COL_ASSIGNED_BOT] = ""
    bots_list = list(get_available_bots().keys()) # ["Bot 1", "Bot 2"]
    if bots_list:
        for i, row in df.iterrows():
            if not row[COL_ASSIGNED_BOT] or row[COL_ASSIGNED_BOT] not in bots_list:
                df.at[i, COL_ASSIGNED_BOT] = bots_list[i % len(bots_list)]
...
def save_block_config_to_sheet(df_ui, blk_name, creds, uid):
    ...
    try:
        ...
        # Đảm bảo cột Bot tồn tại
        if COL_ASSIGNED_BOT not in df_fin.columns: df_fin[COL_ASSIGNED_BOT] = "Bot 1"
...
def rename_block_action(old, new, creds, uid):
    ...
    try:
        ...
        wks.clear(); 
        safe_set_with_dataframe(wks, df, row=1, col=1)
...
def delete_block_direct(blk, creds, uid):
    ...
    try:
        ...
        wks.clear(); 
        safe_set_with_dataframe(wks, df_new, row=1, col=1)
...
def main_ui():
    ...
    with c1: st.title("💎 Kinkin Tool 2.0 (V100 - Stealth Upgrade)", help="V100: Auto Assign Bots"); st.caption(f"User: {uid}")
    ...
    with st.sidebar:
        ...
        with st.expander("⚙️ Manager"):
            ...
            if st.button("✏️ Rename") and rn != sel_blk:
                if rename_block_action(sel_blk, rn, master_creds, uid): st.cache_data.clear(); del st.session_state['df_full_config']; st.session_state['target_block_display'] = rn; st.rerun()
            if st.button("🗑️ Delete"): delete_block_direct(sel_blk, master_creds, uid); st.cache_data.clear(); del st.session_state['df_full_config']; st.rerun()
        st.divider(); 
    ...
    # [V100] Thêm cột Bot_Phu_Trach vào bảng mà không làm hỏng giao diện
    # Tự động load danh sách bot có sẵn
    bot_options = list(get_available_bots().keys())
    
    edt_df = st.data_editor(
        curr_df,
        column_order=[COL_COPY_FLAG, "STT", COL_ASSIGNED_BOT, COL_STATUS, COL_WRITE_MODE, COL_DATA_RANGE, COL_MONTH, COL_SRC_LINK, COL_SRC_SHEET, COL_TGT_LINK, COL_TGT_SHEET, COL_FILTER, COL_HEADER, COL_RESULT, COL_LOG_ROW],
        column_config={
            COL_ASSIGNED_BOT: st.column_config.SelectboxColumn("Bot Phụ Trách", options=bot_options, required=True, width="small"),
            ...
            COL_BLOCK_NAME: None 
        }, use_container_width=True, num_rows="dynamic", key="edt_v100"
    )
        ...
        if st.button("⏩ RUN ALL BLOCKS", use_container_width=True):
            ...
            for idx, blk in enumerate(all_blocks):
                ...
                if isinstance(res, dict):
                    ...
                    if has_change:
                        save_block_config_to_sheet(blk_df, blk, master_creds, uid); main_status.write(f"✅ {blk}: Xong ({tot} dòng).")
    ...
    with c4:
        if st.button("💾 Save Config", use_container_width=True): save_block_config_to_sheet(edt_df, sel_blk, master_creds, uid); st.rerun()
