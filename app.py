...
DEFAULT_BLOCK_NAME = "Block_Mac_Dinh"  # Tên khối dành cho dữ liệu cũ
    ...
    if df is None or df.is_empty():
        try:
            ...
            if data and len(data) > 0:
                df = pl.DataFrame(data[1:], schema=data[0], orient="row"); df = df.select(pl.all().cast(pl.Utf8)); status_msg = f"Thành công"
...
def process_pipeline(rows_to_run, user_id):
    ...
    try:
        ...
        for (target_link, target_sheet), group_rows in grouped_tasks.items():
            ...
            msg_update = ""; success_update = True
            if tasks_list:
...
# --- 4. GIAO DIỆN CHÍNH (UPDATE GIỮ CẤU HÌNH CŨ) ---
def main_ui():
    ...
    def load_data(creds):
        ...
        df = get_as_dataframe(wks, evaluate_formulas=True, dtype=str); df = df.dropna(how='all')
        ...
        # --- LOGIC GIỮ CẤU HÌNH CŨ ---
        # Nếu dòng nào chưa có Block_Name hoặc rỗng -> Gán vào Khối Mặc Định
        ...
        if 'Block_Name' not in df_sys.columns:
            wks_sys.clear(); wks_sys.update([['Block_Name', 'Run_Hour', 'Run_Freq']])
            df_sys = pd.DataFrame(columns=['Block_Name', 'Run_Hour', 'Run_Freq'])
        
        # --- LOGIC ĐẢM BẢO KHỐI CŨ CÓ CẤU HÌNH HẸN GIỜ ---
        # Nếu trong sys_config chưa có "Block_Mac_Dinh", thêm nó vào để nó chạy như cũ (8h Daily)
        if DEFAULT_BLOCK_NAME not in df_sys['Block_Name'].values:
             ...
             wks_sys.append_row([DEFAULT_BLOCK_NAME, '8', 'Hàng ngày'])
             st.toast(f"Đã tự động thêm cấu hình cho {DEFAULT_BLOCK_NAME}!")
    ...
    if 'df_config' not in st.session_state or 'df_sys' not in st.session_state:
        with st.spinner("Đang tải & Di trú cấu hình cũ..."):
            d_conf, d_sys = load_data(creds)
            st.session_state['df_config'] = d_conf; st.session_state['df_sys'] = d_sys
    ...
    if st.button("🚀 CHẠY TẤT CẢ CÁC KHỐI (Tuần tự)", type="primary"):
        ...
        save_data(st.session_state['df_config'], creds); status_text.text("✅ Đã chạy xong tất cả!"); time.sleep(1); st.rerun()
    ...
    for block_name in all_blocks:
        ...
        with st.container(border=True):
            ...
            with c_head: st.subheader(f"📦 Khối: {block_name}" + (" (Cũ/Mặc định)" if block_name == DEFAULT_BLOCK_NAME else ""))
            with c_btn:
                if block_name != DEFAULT_BLOCK_NAME: # Không cho xóa khối mặc định để an toàn
            ...
            if not edited_block_df.equals(df_block_view):
                ...
                st.session_state['df_config'].reset_index(drop=True, inplace=True); st.session_state['df_config']['STT'] = range(1, len(st.session_state['df_config']) + 1)
                            ...
                            if res_map:
                                for idx, row in st.session_state['df_config'].iterrows():
                                    if row['Block_Name'] == block_name and row.get('Link dữ liệu lấy dữ liệu', '') in res_map:
                                        ...
                                        if row['Trạng thái'] != "Đã chốt": st.session_state['df_config'].at[idx, 'Kết quả'] = msg
                                        st.session_state['df_config'].at[idx, 'Dòng dữ liệu'] = rng
                                save_data(st.session_state['df_config'], creds); st.success("Đã xong!"); time.sleep(1); st.rerun()
            with c_scan_b:
