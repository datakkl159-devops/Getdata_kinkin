...
# Tên các Sheet (Phải khớp 100% với file Google Sheet)
        ...
        try:
            ...
            if op == " contains ": 
                ...
                if isinstance(val_resolved, datetime):
                    # [FIX TIMEZONE] Loại bỏ múi giờ để so sánh được với dữ liệu Sheet
...
def process_single_row_automation(row, bot_creds):
    ...
    try:
        ...
        if not existing_vals:
            ...
            # Lọc cột an toàn (chỉ ghi cột có trong nguồn)
...
def parse_weekday(day_str):
    map_day = {
        ...
        'T6': 4, 'THỨ 6': 4, 'FRI': 4,
        'T7': 5, 'THỨ 7': 5, 'SAT': 5,
        'CN': 6, 'CHỦ NHẬT': 6, 'SUN': 6
    }
    key = str(day_str).upper().strip()
    return map_day.get(key, -1)
...
def check_block_due(block_name, sched_df, last_run_time):
    ...
    if l_type == "Chạy theo phút":
        ...
        try:
            minutes_interval = int(val1)
            diff_minutes = (now - last_run_time).total_seconds() / 60
            if diff_minutes >= minutes_interval: 
                return True, f"Interval Met ({int(diff_minutes)}/{minutes_interval})"
        except: pass
        return False, "Wait Interval"
        
    # 2. Logic chung cho các lịch cố định
    try: 
        target_hour = int(val1.split(':')[0])
        target_minute = int(val1.split(':')[1]) if ':' in val1 else 0
    except: return False, "Bad Time"

    # Chỉ chạy nếu giờ hiện tại >= giờ cài đặt
    time_ok = (now.hour > target_hour) or (now.hour == target_hour and now.minute >= target_minute)

    if not time_ok: return False, "Wait Time"
    if has_run_today: return False, "Already Run Today"

    # A. Hàng ngày
    if l_type == "Hàng ngày":
        return True, "Daily Check"

    # B. Hàng tuần
    if l_type == "Hàng tuần":
        target_days = [parse_weekday(d) for d in val2.split(',')]
        if now.weekday() in target_days:
            return True, "Weekly Check"
        return False, "Wrong Weekday"
    ...
    if l_type == "Hàng tháng":
        try:
            ...
            if now.day in target_dates:
                return True, "Monthly Check"
        except: pass
        return False, "Wrong Day of Month"
...
def get_jobs(gc_master):
    try:
        ...
        last_run_map = {}
        try:
            logs = sh.worksheet(SHEET_LOG_NAME).get_all_values()[-500:]
