...
def write_auto_log(creds, history_sheet_id, block_name, status, message):
    try:
        ...
        try: wks = sh.worksheet(SHEET_AUTO_LOG_NAME)
            ...
            wks = sh.add_worksheet(SHEET_AUTO_LOG_NAME, rows=1000, cols=5)
            wks.append_row(["Thời gian (VN)", "Block", "Trạng thái", "Chi tiết", "Ghi chú"])
        tz = pytz.timezone('Asia/Ho_Chi_Minh')
        wks.append_row([datetime.now(tz).strftime("%d/%m/%Y %H:%M:%S"), block_name, status, message, "GitHub Action"])
    except: pass
...
def get_blocks_to_run(creds, history_sheet_id):
    """Xác định các khối cần chạy dựa trên thời gian thực"""
    try:
        gc = gspread.authorize(creds)
        sh = gc.open_by_key(history_sheet_id)
        ...
        # Tự động thêm Default nếu chưa có
        ...
        for _, row in df_sys.iterrows():
            ...
            b_hour = int(row.get('Run_Hour', -1))
            b_freq = row.get('Run_Freq', 'Hàng ngày')
...
def run_auto_job():
    ...
    # 1. Lấy danh sách các khối cần chạy
    blocks_to_run = get_blocks_to_run(creds, HISTORY_SHEET_ID)
    
    if not blocks_to_run:
        print("Không có khối nào hẹn giờ vào lúc này.")
        return

    is_locked, user, _ = get_system_lock(creds, HISTORY_SHEET_ID)
    if is_locked:
        write_auto_log(creds, HISTORY_SHEET_ID, "ALL", "BỎ QUA", f"Lock bởi {user}")
        return
    ...
    try: 
        ...
        # Đảm bảo cột Block_Name tồn tại và điền mặc định
        ...
        # 2. CHẠY LẦN LƯỢT CÁC KHỐI CÓ LỊCH
        for b_name in blocks_to_run:
            print(f"--- Chạy khối: {b_name} ---")
            write_auto_log(creds, HISTORY_SHEET_ID, b_name, "ĐANG CHẠY", "Bắt đầu...")
            
            rows_to_run = df_config[
                (df_config['Block_Name'] == b_name) & 
                (df_config['Trạng thái'] == "Chưa chốt & đang cập nhật")
            ].to_dict('records')

            if not rows_to_run:
                write_auto_log(creds, HISTORY_SHEET_ID, b_name, "XONG", "Không có dòng chờ xử lý.")
                continue
            ...
            if all_block_success:
                ...
                df_config.loc[mask, 'Kết quả'] = msg_sum
                write_auto_log(creds, HISTORY_SHEET_ID, b_name, "THÀNH CÔNG", msg_sum)
            else:
                write_auto_log(creds, HISTORY_SHEET_ID, b_name, "CÓ LỖI", msg_sum)
